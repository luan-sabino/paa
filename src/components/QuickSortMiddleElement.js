...
export default function QuickSortMiddleElement(){
    ...
    const title = 'Quick Sorte Elemento Central como Pivô';

    function swap(vetor, leftIndex, rightIndex){
        var temp = vetor[leftIndex];
        vetor[leftIndex] = vetor[rightIndex];
        vetor[rightIndex] = temp;
    }

    function partition(vetor, left, right) {
    ...
    function quickSort(vetor, left, right) {
        var index;
        if (vetor.length > 1) {
            index = partition(vetor, left, right);
            if (left < index - 1) {
                quickSort(vetor, left, index - 1);
            }
            if (index < right) {
                quickSort(vetor, index, right);
            }
        }
        return vetor;
    }
    ...
    function Sorting(vetor){
        ...
        let result = quickSort(vetor, 0, vetor.length - 1);
